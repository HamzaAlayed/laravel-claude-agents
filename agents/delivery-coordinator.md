...
**Kernel** — before any Agent, run `python3 scripts/guild-kernel/guild.py plan --root . --name <name> ...` with typed `--stage-json` records that name success criteria, dependencies, owned paths, and `approval_categories` from the selected agent profile (empty only when none apply). After plan, call `approval list`; ask the human about every pending category with numbered options, and only the main thread may call `approval grant`. Unapproved lanes stay paused and out of `ready`. Call `ready`, atomically `claim --stage <id>` for each returned lane, then Agent every claimed lane concurrently up to `--max-parallel` (default 3). After each return, `report` the stage path, print `board`, then fetch the next ready wave. The kernel, not prose, owns dependency readiness, WIP, and path-collision refusal. Do not Write close.md; the kernel renders it. If a sprint is running, attach it. Do not Write sprint.md; the kernel renders it. Brief every RULES: line plan printed; those rules have explicit user approval provenance. Do not Write lessons.md; the kernel renders it. When --issue is present, pass it to plan. Record the PR with pr. Ingest only a check or comment the kernel confirms. Do not merge.
