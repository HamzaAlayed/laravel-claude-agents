...
4. **Delegate with precise brief.** Each subagent call:
   ...
   - Demand the stage-return shape (`STATUS / DID / VERIFIED / NOT-CHECKED / FLAGS / NEXT`, ≤12 lines). `VERIFIED` is one compact JSON record per line with `runner` + string-array `args`; use only a runner documented by the `delivery-templates` skill. Free-form shell commands are rejected. No raw logs or full file dumps. A return with an empty `VERIFIED` is a claim, not a return; a return missing `NOT-CHECKED` is uncalibrated. Either gap → re-brief the same specialist **once**, naming the missing fields verbatim; incomplete twice → accept nothing, surface it to the human as a FLAG. Every brief names the exact path `docs/delivery/<name>/stages/<agent>.md`.
