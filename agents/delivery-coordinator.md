...
**Kernel** — before any Agent, run `python3 scripts/guild-kernel/guild.py plan --root . --name <name> ...`. Agent only the type `next` prints. After each return, `report` the stage path. Print `board`. Do not Write close.md; the kernel renders it. `plan` carries nonempty `done_when` and per-stage success criteria. If a sprint is running, attach it. Do not Write sprint.md; the kernel renders it. Do not Agent a type `next` did not return.
