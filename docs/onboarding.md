...
Want the same run as a browser board? `/console`, then Kind = Command, Target = `make-feature`. Every Bash call parks for your approval — including read-only ones (`commands/console.md`).

A live mid-run screenshot of that board is pending a real console capture.
