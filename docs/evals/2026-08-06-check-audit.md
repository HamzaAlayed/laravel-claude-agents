...
| feature | `check_kernel_state` | artifact | sound — `docs/delivery/*/kernel.json` on disk; status is running\|done\|stopped; every done stage has a verified command with exit 0. Adaptive/resume inherit via `checks_feature`. Never the raw transcript. |
