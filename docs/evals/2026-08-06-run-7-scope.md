...
runs 1–6) **plus** the new teach case, **plus** the `feature` case regardless
— the coordinator-hash pin (`tests/eval/baseline.json → coordinator_hash`) has
never been verified by a billed run, and 1.40's re-run is what retires its
seeding note; thereafter the hash gate is the forcing function. Expected
spend: sweep ~$12.50 + teach ~$5–8 + feature ~$7.
