...
| agents/delivery-coordinator.md | 110, 135 | Rule "Never patch a subagent's work" / "Builder/reviewer work yourself? … Stop. Delegate." governs main-thread behavior. It is echoed in 5 of 9 command bodies (commands/optimize-query.md:63, commands/review-pr.md:63, commands/upgrade-laravel.md:13,97, commands/audit-n-plus-one.md:71, commands/add-test.md:71) but is entirely absent from commands/make-feature.md, commands/ship-checklist.md, commands/add-policy.md, commands/refactor-to-action.md, and from the shared Interface block itself — so 4 of 9 commands have no stated guard against the main thread patching a specialist's work directly. | Blocking | Move "never patch a subagent's work — re-brief or escalate instead" into the shared Interface block so all 9 commands carry it uniformly. |
