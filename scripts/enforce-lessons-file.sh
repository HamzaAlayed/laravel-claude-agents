#!/usr/bin/env bash
# Bounce Writes/Edits of docs/team/lessons.md unless the payload is helper
...
deny() {
  echo "blocked: docs/team/lessons.md must be helper shape." >&2
  echo "copy the kernel lessons view; lessons.md is a kernel view, not a journal." >&2
  exit 2
}

deny_bash() {
  echo "blocked: Bash must not write docs/team/lessons.md." >&2
  echo "use the Write tool; the kernel renders lessons.md." >&2
  exit 2
}

# True when flattened Bash writes docs/team/lessons.md via >, >>, tee, or
# a heredoc onto that path. Does not parse heredoc labels. Does not match
# the kernel lessons view.
writes_lessons_file() {
  local cmd="$1"
  # Relative or absolute docs/team/lessons.md — not the skill stub.
  local path='docs/team/lessons\.md'
...
# when the payload looks like a lessons.md — do not try to score labels.
# Stronger than write-operator-only: any docs/team/lessons.md still denies
# (Write|Edit journal and Bash writes).
if [ "$PATH_TARGET" = "$INPUT" ]; then
  if printf '%s' "$PATH_TARGET" | grep -qE 'docs/team' \
     && printf '%s' "$PATH_TARGET" | grep -qE 'lessons\.md'; then
    deny
  fi
  exit 0
fi
...
if [ -n "$COMMAND" ]; then
  ...
  if writes_lessons_file "$FLAT"; then
    deny_bash
  fi
  exit 0
fi

# Lessons view only — not conventions.md.
if ! printf '%s' "$PATH_TARGET" | grep -qE '(^|/)docs/team/lessons\.md$'; then
  exit 0
fi
