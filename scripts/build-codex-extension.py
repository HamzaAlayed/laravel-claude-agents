...
AGENT_DELIVERY_NEW = (
    ...
    "hooks below. The full 18-agent specialist team runs on Claude Code and Gemini CLI."
)
...
def build_hooks():
    ...
    write(out, txt)
    os.chmod(out, 0o755)
    with open(os.path.join(ROOT, "scripts", "enforce-lessons-file.sh")) as f:
        txt = sanitize(f.read())
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          ...
          { "type": "command", "command": "%(r)s/.codex/hooks/enforce-lessons-file.sh", "statusMessage": "Enforcing lessons.md helper shape" }
        ]
      },
      {
