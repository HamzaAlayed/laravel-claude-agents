#!/usr/bin/env bash
# Bounce Writes/Edits of docs/sprints/*/sprint.md unless the payload is helper
...
deny() {
  echo "blocked: docs/sprints/*/sprint.md must be helper shape." >&2
  echo "copy the kernel sprint view; sprint.md is a kernel view, not a journal." >&2
  exit 2
}

deny_bash() {
  echo "blocked: Bash must not write docs/sprints/*/sprint.md." >&2
  echo "use the Write tool; the kernel renders sprint.md." >&2
  exit 2
}

# True when flattened Bash writes docs/sprints/*/sprint.md via >, >>, tee, or
# a heredoc onto that path. Does not parse heredoc labels. Does not match
# the kernel sprint view.
writes_sprint_file() {
  local cmd="$1"
  # Relative or absolute docs/sprints/<id>/sprint.md — not the skill stub.
  local path='docs/sprints/[^ ;&|"'"'"']+/sprint\.md'
...
# when the payload looks like a sprint.md — do not try to score labels.
# Stronger than write-operator-only: any docs/sprints + sprint.md still denies
# (Write|Edit journal and Bash writes).
if [ "$PATH_TARGET" = "$INPUT" ]; then
  if printf '%s' "$PATH_TARGET" | grep -qE 'docs/sprints' \
     && printf '%s' "$PATH_TARGET" | grep -qE 'sprint\.md'; then
    deny
  fi
  exit 0
fi
...
if [ -n "$COMMAND" ]; then
  ...
  if writes_sprint_file "$FLAT"; then
    deny_bash
  fi
  exit 0
fi

# Sprint board only — not retro.md.
if ! printf '%s' "$PATH_TARGET" | grep -qE '(^|/)docs/sprints/.+/sprint\.md'; then
  exit 0
fi

BODY="$(extract_body)"

if printf '%s' "$BODY" | grep -qE '^GOAL:' \
   && printf '%s' "$BODY" | grep -qE '^WIP:' \
   && printf '%s' "$BODY" | grep -qE '^BOARD:' \
