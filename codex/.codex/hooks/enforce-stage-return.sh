#!/usr/bin/env bash
# Bounce Writes/Edits of docs/delivery/*/stages/*.md unless the payload is
# helper shape, and deny Bash that writes that path. Wire this via a
# PreToolUse hook on Write, Edit, and Bash. Exit 2 to block.
#
...
deny() {
  echo "blocked: docs/delivery/*/stages/*.md must be helper shape." >&2
  echo "copy skills/delivery-templates/stage-return.md; stage returns are not journals." >&2
  exit 2
}

deny_bash() {
  echo "blocked: Bash must not write docs/delivery/*/stages/*.md." >&2
  echo "use the Write tool; copy skills/delivery-templates/stage-return.md." >&2
  exit 2
}

# True when flattened Bash writes docs/delivery/*/stages/*.md via >, >>, tee, or
# a heredoc onto that path. Does not parse heredoc labels. Does not match
# skills/delivery-templates/stage-return.md.
writes_delivery_stage() {
  local cmd="$1"
  # Relative or absolute docs/delivery/<feature>/stages/<agent>.md — not the skill stub.
  local path='docs/delivery/[^ ;&|"'"'"']+/stages/[^ ;&|"'"'"']+\.md'
...
# when the payload looks like a delivery stage return — do not try to score labels.
# Stronger than write-operator-only: any docs/delivery + stages + .md still denies
# (Write|Edit journal and Bash writes).
if [ "$PATH_TARGET" = "$INPUT" ]; then
  if printf '%s' "$PATH_TARGET" | grep -qE 'docs/delivery' \
     && printf '%s' "$PATH_TARGET" | grep -qE '/stages/' \
     && printf '%s' "$PATH_TARGET" | grep -qE '\.md'; then
    deny
  fi
  exit 0
fi
...
if [ -n "$COMMAND" ]; then
  ...
  if writes_delivery_stage "$FLAT"; then
    deny_bash
  fi
  exit 0
fi

# Delivery artifact only — not skills/delivery-templates/stage-return.md.
if ! printf '%s' "$PATH_TARGET" | grep -qE '(^|/)docs/delivery/.+/stages/.+\.md'; then
  exit 0
fi

BODY="$(extract_body)"

if printf '%s' "$BODY" | grep -qE '^STATUS:' \
   && printf '%s' "$BODY" | grep -qE '^DID:' \
   && printf '%s' "$BODY" | grep -qE '^VERIFIED:' \
   && printf '%s' "$BODY" | grep -qE '^NOT-CHECKED:' \
   && printf '%s' "$BODY" | grep -qE '^FLAGS:' \
   && printf '%s' "$BODY" | grep -qE '^NEXT:'; then
  exit 0
fi
