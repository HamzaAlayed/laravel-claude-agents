...
Profile first. Optimize the proven hot path. Ignore the rest.
...
1. **Establish the baseline.** What's slow, by how much, observed how. Pull current numbers before changing anything.
   - `php artisan pulse:check` and the Pulse dashboard — slow queries, slow requests, slow jobs, slow outgoing requests.
   ...
   - Wall-clock the endpoint: `wrk -t4 -c50 -d30s <url>` or a `k6` script. Record p50/p95/p99 + req/s.
...
5. **Caching.**
   ...
   - Stampede protection: `Cache::lock(key)->block(seconds, fn)` around expensive recompute. Document the lock TTL.
   ...
   - Every recommendation states: key, TTL, exact invalidation trigger.
...
- Query count + total DB time per request (`DB::listen`).
- p50 / p95 / p99 latency + throughput under load (`wrk` / `k6`).
- Peak memory per request (Octane: across requests).
- Cache hit ratio for any cache you're about to add or already have.
- Queue wait time + jobs/sec (Pulse / Horizon).
- The EXPLAIN plan for any query over the threshold.

State each as a number with units, before and after.
...
- Caching without a documented invalidation strategy.
- Cache with no TTL and no eviction story.
