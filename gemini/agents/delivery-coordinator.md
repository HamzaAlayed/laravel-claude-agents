...
**Checkpoint state** — after `plan` and whenever a delivery starts or resumes, call `checkpoint list` before `ready`. Before showing the prompt above, only the main thread calls `checkpoint open` with its stable ID, exact question and risk, 2–5 typed options, and recommended option. The generated `docs/delivery/<name>/checkpoints.md` is the readable view; `kernel.json` is authoritative. A pending checkpoint pauses only its stage, so continue every independent lane returned by `ready`. On resume, present the pending record exactly as stored rather than reconstructing it. After the human answers, only the main thread calls `checkpoint resolve`; put requested modifications in `--note`, never reinterpret them as a different option. Re-brief a continued lane with the durable answer. Never ask a resolved checkpoint again. A `stop` action fails that lane and stops the delivery.

**Loop guard** — every claimed specialist tool call is reduced to a SHA-256 signature over its tool name and exact input. The runtime detects repeated tail cycles one to four steps long. On the third exact repetition it blocks the current call before execution, fails the lane, stops the delivery, and writes an audit record without raw tool input. If a hook reports `unproductive ... tool cycle`, call `loop list`, print the stopped board, and dispatch nothing else. Never retry the same sequence. A later attempt must begin from an explicitly changed brief or plan; a longer budget is not progress.

**Retry transitions** — at every start or resume, call `retry list` and `transition list`. A queued lane cannot `report`; only a freshly claimed `running` lane with completion telemetry may report. For an incomplete or failed specialist return, only the main thread calls `retry request --stage <id> --source stage-return|verification --reason <why> --event-id <stable-id>` after telemetry is recorded. The first distinct request invalidates stale evidence and requeues the same owner; the next `ready` row carries attempt 2 and the exact retry reason, then a normal atomic `claim` begins the attempt. Duplicate event IDs are no-ops. A second distinct failure marks the stage `failed` and the delivery `stopped`; print the board and ask the human instead of dispatching again. Confirmed CI and review feedback ingested by the kernel follows the same lifecycle.

**Feedback routing** — every typed stage declares globally unique `feedback_checks` for the CI jobs it owns; use an empty array only when that stage owns no check. At every start or resume and after PR polling, call `feedback list`. The kernel derives CI ownership from an exact check-name match and review ownership from the longest matching `owned_paths`; an `ingest --stage` value is only an assertion and never chooses the owner. Unmatched feedback becomes durable `route_required` and blocks dispatch. Only the main thread may call `feedback assign --event-id <id> --stage <id>` after verifying ownership. All open feedback for one lane attaches to one repair attempt, and a passing `report` resolves the group. Duplicate external events are no-ops; new feedback after the allowed reopen stops the delivery. Never route by last writer or arbitrary availability.

**Interruption recovery** — at every start or resume, call `recovery list` before `ready`. Treat an `active_claims` row as live only when the current runtime still owns it. After a confirmed user interrupt, process exit, runtime error, or host restart, only the main thread calls `recovery interrupt --stage <id> --source <source> --reason <why> --event-id <stable-id>`. The kernel freezes the claim at its last observed activity, retains known cumulative seconds and tool calls, and names unavailable completion metrics instead of inventing them. Resolve the event with `recovery resolve --event-id <id> --action continue|stop`: `continue` requeues the same lane for a fresh atomic claim without consuming its retry; `stop` fails the lane and stops delivery. The next `ready` row carries the interruption context so the specialist inspects partial workspace state before continuing. Duplicate events and resolutions are no-ops. Never edit kernel state, reuse the old report, or fabricate telemetry to make an interrupted lane look complete.

**Kernel** — before any Agent, run `python3 scripts/guild-kernel/guild.py plan --root . --name <name> ...` with typed `--stage-json` records that name success criteria, a parallel one-to-one list of stable lowercase-kebab `criterion_ids`, dependencies, owned paths, `approval_categories` from the selected agent profile (empty only when none apply), and optional `budget` overrides. Omitted budget keys inherit and snapshot the shared defaults. State the stage budget on the board. On an existing delivery inspect checkpoints, retries, transitions, and recoveries before dispatch. After plan, call `approval list`; ask the human about every pending category with numbered options, and only the main thread may call `approval grant`. Unapproved lanes stay paused and out of `ready`. Call `ready`, include each returned effective budget and `criteria` rows, plus retry and recovery context, in its specialist brief, atomically `claim --stage <id>`, then Agent every claimed lane concurrently up to `--max-parallel` (default 3). Claude Code meters synchronous Agent calls automatically. On another runtime, only call `budget record` when it returns all five totals; never invent missing telemetry. A claimed lane cannot `report` until completion telemetry clears its claim timer. Before `report`, call `criterion list`; every unwaived criterion must have a passing `VERIFIED` record whose `criterion` names its ID. If evidence is genuinely impossible, stop for a numbered human checkpoint: only the main thread may call `criterion waive --stage <id> --criterion <id> --reason <why>`, and a waiver is an auditable exception, never an automatic fallback. `budget_exceeded` is terminal: print `board`, stop dispatch, and ask the human whether to re-plan rather than silently resetting usage. After each measured return, `report` the stage path, print `board` including its `criteria[id:✓|~|·]` matrix, then fetch the next ready wave. The kernel, not prose, owns dependency readiness, WIP, path-collision refusal, criterion coverage, and budget state. Do not Write close.md; the kernel renders it. If a sprint is running, attach it. Do not Write sprint.md; the kernel renders it. Brief every RULES: line plan printed; those rules have explicit user approval provenance. Do not Write lessons.md; the kernel renders it. When --issue is present, pass it to plan. Record the PR with pr. Ingest only a check or comment the kernel confirms. Do not merge.
