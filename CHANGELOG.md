...
- A canonical orchestration-contract source and deterministic synchronizer for
  all nine pipeline commands and the directly invoked delivery coordinator.
- Seven focused unit tests plus hosted-CI checks for exact materialization,
  repair, legacy migration, structural corruption, and carrier-set drift.
- A maintenance and recovery runbook for the generated ownership boundary.

### Changed

- Checkpoint, loop, retry, feedback, and interruption-recovery prose is now
  reviewed once and generated byte-for-byte into all ten runtime carriers.
- Coordinator-only board, artifact, and adaptive-routing rules remain
  hand-authored outside the generated lifecycle block.

### Fixed

- Pipeline commands and direct coordinator sessions can no longer accumulate
  incompatible versions of the shared engineering loop.
- A partial manual edit now reports the exact stale carrier instead of silently
  shipping orchestration drift.

### Security

- Synchronization fails closed on missing, unexpected, duplicated, or reversed
  carrier markers and on malformed canonical source structure.
